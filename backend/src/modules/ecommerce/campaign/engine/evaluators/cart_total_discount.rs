...
use crate::modules::ecommerce::campaign::scenario::{CartTotalDiscountParams, RewardType, ScenarioType};
...
pub fn eval_cart_total_discount(
    params: &CartTotalDiscountParams,
    cart_total: Decimal,
    cart_currency: &str,
    exchange_rates: Option<&ExchangeRateModel>,
) -> Option<DiscountResult> {
    let min_cart_total_converted = convert_amount(params.min_cart_total, &params.currency, cart_currency, exchange_rates);

    if cart_total < min_cart_total_converted {
        return None;
    }
    ...
    match &params.reward_type {
        RewardType::FixedDiscount => {
            ...
            Some(DiscountResult {
                campaign_id: 0,
                coupon_id: None,
                scenario_type: ScenarioType::CartTotalDiscount,
                ...
                description: format!(
                    "Sepet toplamı {} {} üzeri, {} {} indirim",
                    params.min_cart_total, params.currency, params.reward_value, params.currency
                ),
                cart_id: 0,
            })
        }
        RewardType::PercentDiscount => {
            ...
            Some(DiscountResult {
                ...
                description: format!(
                    "Sepet toplamı {} {} üzeri, %{} indirim",
                    params.min_cart_total, params.currency, params.reward_value
                ),
                cart_id: 0,
            })
        }
