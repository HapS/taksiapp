...
pub async fn get_product(
    ...
    let model = match (id, slug) {
        ...
        (_, Some(slug)) => filter.all(db).await?.into_iter().find(|p| {
            let page_slug = crate::modules::content::helpers::product_helper::get_string_from_json(
                &p.data, lang, "slug",
            )
            .unwrap_or_default();
            page_slug == slug
    ...
    Ok(
        crate::modules::content::helpers::product_helper::to_product_get_response(
            &model,
            lang,
            db,
        )
...
pub async fn list_products(
    ...
    let tags_map = if !content_ids.is_empty() {
        crate::modules::content::helpers::product_helper::fetch_tags_for_contents(
            db,
            &content_ids,
            lang,
        )
    ...
    let attribute_terms_map = if !attr_ids.is_empty() {
        crate::modules::content::helpers::product_helper::fetch_terms_by_ids(db, &attr_ids, lang)
            .await
            .unwrap_or_default()
    } else {
        std::collections::HashMap::new()
    ...
    for product in items.iter() {
        if crate::modules::content::helpers::product_helper::has_content_in_language(
            &product.data,
            lang,
        ) {
            results.push(
                crate::modules::content::helpers::product_helper::to_product_list_response(
                    db,
                    product,
                    lang,
                    &tags_map,
...
pub async fn list_products_paginated(
    ...
    (
        Vec<crate::modules::content::helpers::product_helper::ProductResponse>,
        i64,
    ),
    ServiceError,
> {
    ...
    let total_count: i64 = Content::find()
        .filter(ContentColumn::ContentType.eq("product"))
        .filter(ContentColumn::Publish.eq(true))
        .filter(ContentColumn::DeletedAt.is_null())
        .count(db)
...
pub async fn get_products_map_by_ids(
    ...
    std::collections::HashMap<
        i64,
        crate::modules::content::helpers::product_helper::ProductResponse,
    >,
    ServiceError,
> {
    use crate::modules::content::helpers::product_helper as helper;

    if ids.is_empty() {
        return Ok(std::collections::HashMap::new());
    }

    // Fetch contents in a single query
    let items = Content::find()
        .filter(ContentColumn::ContentType.eq("product"))
        .filter(ContentColumn::Publish.eq(true))
        .filter(ContentColumn::DeletedAt.is_null())
    ...
    // Pre-fetch tags for these contents
    let content_ids: Vec<i64> = items.iter().map(|p| p.id).collect();
    let tags_map = if !content_ids.is_empty() {
    ...
    let attribute_terms_map = if !attr_ids.is_empty() {
        helper::fetch_terms_by_ids(db, &attr_ids, lang)
            .await
            .unwrap_or_default()
    } else {
        std::collections::HashMap::new()
    ...
    // Build ProductResponse for each content
    let mut results: std::collections::HashMap<i64, helper::ProductResponse> =
        std::collections::HashMap::new();

    for content in items.into_iter() {
        if helper::has_content_in_language(&content.data, lang) {
            let resp = helper::to_product_list_response(
                db,
                &content,
                lang,
                &tags_map,
...
/// Paginated version of list_products_category_with_filters
pub async fn list_products_category_with_filters_paginated(
    db: &sea_orm::DatabaseConnection,
    lang: &str,
    category_id: Option<i64>,
    attribute_filters: std::collections::HashMap<String, Vec<i64>>,
...
> {
    ...
    let total_count = content_ids.len() as i64;

    // Apply pagination
    let offset = (page.saturating_sub(1)) * per_page;
    let paginated_ids: Vec<i64> = content_ids
        .iter()
        .skip(offset)
        .take(per_page)
        .cloned()
    ...
    let items = if !paginated_ids.is_empty() {
        Content::find()
            .filter(ContentColumn::ContentType.eq("product"))
            .filter(ContentColumn::Publish.eq(true))
            .filter(ContentColumn::DeletedAt.is_null())
            .filter(ContentColumn::Id.is_in(paginated_ids.clone()))
            .order_by_asc(ContentColumn::OrderId)
            .all(db)
            .await?
    } else {
    ...
    // Pre-fetch all tags for all pages in a single batch query (N+1 fix)
    let tags_map = if !paginated_ids.is_empty() {
        crate::modules::content::helpers::product_helper::fetch_tags_for_contents(
            db,
            &paginated_ids,
            lang,
