...
impl TimelineHelper {
    ...
    pub async fn create_order_event(
        ...
        let (title_tr, title_en, description_tr, description_en) = match event_type {
            TimelineEventType::OrderCreated => (
                "Sipariş oluşturuldu",
                "Order created",
                Some("Siparişiniz başarıyla oluşturuldu ve işleme alındı"),
                Some("Your order has been successfully created and is being processed"),
            ),
            TimelineEventType::OrderStatusChanged => (
                "Sipariş durumu güncellendi",
                "Order status updated",
                Some("Siparişinizin durumu güncellendi"),
                Some("Your order status has been updated"),
            ),
            TimelineEventType::OrderShipped => (
                "Sipariş kargoya verildi",
                "Order shipped",
                Some("Siparişiniz kargo firmasına teslim edildi"),
                Some("Your order has been handed over to the shipping company"),
            ),
            TimelineEventType::OrderDelivered => (
                "Sipariş teslim edildi",
                "Order delivered",
                Some("Siparişiniz başarıyla teslim edildi"),
                Some("Your order has been successfully delivered"),
            ),
            TimelineEventType::OrderCancelled => (
                "Sipariş iptal edildi",
                "Order cancelled",
                Some("Siparişiniz iptal edildi"),
                Some("Your order has been cancelled"),
            ),
            _ => return Err("Geçersiz sipariş event tipi".into()),
        };
        ...
        if let (Some(desc_tr), Some(desc_en)) = (description_tr, description_en) {
            description.insert("tr".to_string(), desc_tr.to_string());
            description.insert("en".to_string(), desc_en.to_string());
        }

        let request = CreateTimelineEventRequest {
            module_type: "ecommerce".to_string(),
            content_type: "order".to_string(),
            content_id: order_id,
            event_type,
            title,
            ...
            icon: None,  // Default icon kullanılacak
            color: None, // Default color kullanılacak
            user_id,
            admin_user_id,
            metadata,
            is_public: Some(true),
